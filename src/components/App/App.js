...
class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            dataRoute: 'http://192.168.0.103:3009/data',
            regions: regions,
            focus: {
                col: 3,
                row: 3,
  ...
  render() {
    return (
      <div className={`m sans-serif flex flex-nowrap flex-column absolute absolute--fill white `}>
          {this.state.hasData && this.state.regions.map((regions, row) => <RegionRow regions={regions} key={row} focus={this.state.focus} row={row + 1} data={this.state.data} components={componentMap} />)}
      </div>
    );
  }
}
